...
ee_data_image <- function(datasetID = "CGIAR/SRTM90_V4",
  ...
  match.arg(spatialReducer,
            choices = c("mean", "median", "min", "max", "mode"))
  is_type(resolution, "numeric", NULL)
  is_type(bandSelection, "character", NULL)
  if (length(bandSelection) > 1 &
      !is.vector(bandSelection))
    stop(
      "bandSelection is not a vector. \nIf you want to select multiple bands, pass them inside a vector.",
      call. = F
    )
  ...
  product_name_new <-
    paste0(gsub("/", "-", datasetID), "_", "s-", spatialReducer)
  productInfo <- list(
    datasetID = datasetID,
...
ee_data_collection <- function(datasetID = "UCSB-CHG/CHIRPS/DAILY",
  ...
  match.arg(spatialReducer,
            choices = c("mean", "median", "min", "max", "mode"))
  match.arg(temporalReducer,
            choices = c("mean", "median", "min", "max", "mode", "sum"))
  ...
  if (is.na(timeStart))
    stop(paste(timeStart, "is not a valid Date"), call. = F)
  if (is.na(timeEnd))
    stop(paste(timeEnd, "is not a valid Date"), call. = F)
  
  if (identical(timeStart, timeEnd))
    stop(
      "timeStart and timeEnd have the identical date: ",
      timeStart,
      "\nTo select only the single day ",
      timeStart,
      " use the date range of timeStart: ",
      timeStart,
      " and timeEnd: ",
      timeEnd + 1,
