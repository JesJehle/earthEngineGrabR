...
ee_grab <- function(data = NULL,
  ...
  if (is.null(data) |
      !is.list(data[1]))
    stop(
      "No data specified. \nPlease specify your requested data with a single ee_data_image() or ee_data_collection() functions.",
      call. = F
    )
  
  earthEngineGrabR:::activate_environments("earthEngineGrabR")
  try(googledrive::drive_rm("earthEngineGrabR-tmp", verbose = F),
      silent = T)
  ...
  download_data(ee_response = ee_response,
                verbose = verbose,
